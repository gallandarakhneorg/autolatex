...
svg, GNU plot, ...) dans un PDF ou un fichier PNG.
...
=item B<GNU plot (plot) vers TeX dans du Portable Document Format (pdf+tex)>
...
=item B<Scalable Vector Graphic avec claques (svg) vers TeX dans des calques Beamer>

PDF part:

=over 12

=item Nom :                              svg2pdf+layers+tex
...
C'est l'interface I<officielle> et "à jour" de AutoLaTeX. Un greffon pour l'éditeur léger Gedit <https://projects.gnome.org/gedit> est fourni. Vous pouvez lancer AutoLaTeX et le configurer grâce aux differents panneaux fournis.

=item B<Sublime Text 2:>

Un greffon pour l'éditeur Sublime Text 2 <http://www.sublimetext.com/2> est fourni. Vous pouvez lancer AutoLaTeX grâce aux outils de construction/compilation de Sublime Text.

=item B<TeXworks:>

Il n'y a pas de greffon particulier pour TeXworks. Pour utiliser AutoLaTeX dans cet éditeur, vous devez ajouter un outil de composition dans la boîte de dialogue "Préférences", ou dans le fichier de configuration F<tools.ini> dans le dossier de TeXworks.
...
Il n'y a pas de greffon particulier pour Kile. Pour utiliser AutoLaTeX dans cet éditeur, vous devez ajouter un outil de compilation dans la boîte de dialogue "Configure Kile", ou dans le fichier de configuration F<kilerc> dans le dossier de de configuration de KDE.
...
=item B<Application Gtk (obsolète)>:

Il s'agit de la première interface de AutoLaTeX, écrite en Perl et basée sur Gtk. Elle est maintenant considérée comme I<obsolète>, même si elle est toujours maintenue. Vous pouvez lancer AutoLaTeX et le configurer grâce aux differents panneaux fournis.
