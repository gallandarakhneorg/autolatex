...
As a quick example, consider a project, which has a single LaTeX file C<mydoc.tex>, as its input. Without AutoLaTeX, to produce a C<.pdf> file you might use the following sequence of commands:
...
AutoLaTeX provides a fixed set of targets, the arguments to pass to AutoLaTeX to run a module, for all projects. The default target is B<all>.
...
Same as B<view>, except that the viewer is launched only if the configuration or the
CLI is enabling the viewer.
...
Cleans the current working directory by removing all LaTeX temp files and other
temp files created during processing of the project.

=item B<cleanall>

Same as clean. In addition, emacs ~ files and other backup files are removed.
The generated figures and the produced PDF are also removed.
...
for the user level.
The project configuration file is C<path/to/project/.autolatex_project.cfg> on
Unix platforms, and C<path\to\project\autolatex_project.cfg> on other platforms.
The default user configuration file is C<$HOME/.autolatex>
on Unix platforms, C<C:\Documents and Settings\User\Local Settings\Application Data\autolatex.conf>
on Windows platforms, and C<$HOME/autolatex.conf> on other plateforms.
...
AutoLaTeX will search for a TeX file in the current directory.
...
which must be processed by the translators. Each time this
option is put on the command line, a directory is added inside
the list of the directories to explore.
...
a value, the F<style_file> value will be assumed to be an .ist file to
pass to MakeIndex.
...
Use the historical LaTeX command: C<latex>.

=item B<--lualatex>

Use the LaTeX command: C<lualatex>.
...
Use the LaTeX command: C<pdflatex>.
...
AutoLaTeX should be not verbose (see B<-v> for changing the verbose level).
...
If S<translator> is given, it is the name of the translator for which the value
must ve overridden. If S<translator> is not given then "generation" is the default
prefix.

=item B<-v>

Each time this option was specified, AutoLaTeX is more verbose. Note that 1) if you put
5 times the B<-v> option on the command line, AutoLaTeX will consider the Perl warnings
as errors; 2) if you put 6 times the B<-v> option on the command line, AutoLaTeX is
stopping immediately, and is displaying the current configuration in memory.
...
Use the LaTeX command: C<xelatex>.
...
Each supported translator is described inside a .transdef file. This file contains
the definition of the variables for the shell command line to launch or the
Perl code to use. To create a new translator, we recommend to copy/paste an
existing C<.transdef> file and change its content.
Even if you excluded a translator from the the command line, it is automatically
included by AutoLaTeX when it is invoked by an included translator.
...
=item Input format:                      .dia_tex .diat .diatex .dia+tex
...
=item Input format:                      .fig_tex .figt .figtex .fig+tex
...
=item B<GNU plot (plot) to TeX embedded in Portable Document Format (pdf+tex)>
...
=item Input format:                      .plot_tex .plottex .plott .plot+tex
...
=item Name:                              svg2pdf+tex

=item Use external converter:            inkscape

=item Use translator:                    eps2pdf

=item Input format:                      .svg_tex .svgt .svgtex .svg+tex
...
=item Name:                              svg2pdf+layers

=item Use external converter:            inkscape

=item Use translator:                    

=item Input format:                      .svgl .svg_layers .svg+layers
...
=item Name:                              svg2pdf+layers+tex

=item Use external converter:            inkscape

=item Use translator:                    

=item Input format:                      .svg_layerstex .svglt .svglayerstex .svg+layers+tex
...
AutoLaTeX provides a LaTeX style called C<autolatex.sty>. It provides the following functions:

=over 4

=item B<\includegraphicswtex[options]{filename}>

include a figure with combined TeX macros. The filename must have one of the following extensions: C<.pdftex_t>, C<.pdftex_t>, C<.pstex_t>, C<.pdf_tex>, C<.ps_tex>. The B<options> must one of: C<width=XX>, or C<height=XX>; where C<XX> is a length.
...
a Beamer-compatible environment. The filename must have one of the following extensions: C<.pdftex_t>, C<.pdftex_t>, C<.pstex_t>, C<.pdf_tex>, C<.ps_tex>. The B<options> must one of: C<width=XX>, or C<height=XX>; where C<XX> is a length.

By default, a layer is replacing the previous layer when it is displayed. You could change
the overlay specification by adding the string C<"spec"> in the title of the layer in your SVG
editor. The C<spec> part is the specification of the slide numbers on which the layer should
appear in Beamer. For example, C<2> means 'only on the slide 2'; C<6-> means 'from slide 6 to the end'.
...
The filename must have one of the following extensions: C<.pdftex_t>, C<.pdftex_t>, C<.pstex_t>, C<.pdf_tex>, C<.ps_tex>. The B<options> must one of: C<width=XX>, or C<height=XX>; where C<XX> is a length.
...
=item C<latex> - use C<latex>

=item C<pdflatex> - use C<pdflatex>

=item C<xelatex> - use C<xelatex>

=item C<lualatex> - use C<lualatex>
...
Copyright (c)  1998-2013  Stephane Galland E<lt>galland@arakhne.orgE<gt>.
