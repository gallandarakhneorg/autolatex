...
class Panel(Gtk.Table):
	...
	def __init__(self, settings, window):
		...
		ui_table.attach(self._ui_use_synctex, 
				...
				Gtk.AttachOptions.SHRINK, # y options
				5,5) # horizontal and vertical paddings
		ui_label = Gtk.Label(_T("Update the TeX files with the SyncTeX flags"))
		ui_table.attach(ui_label, 
				0,1,1,2, # left, right, top and bottom columns
				Gtk.AttachOptions.SHRINK, # x options
				Gtk.AttachOptions.SHRINK, # y options
				5,5) # horizontal and vertical paddings
		self._ui_update_synctex = Gtk.Switch()
		ui_table.attach(self._ui_update_synctex, 
				1,2,1,2, # left, right, top and bottom columns
				Gtk.AttachOptions.FILL|Gtk.AttachOptions.EXPAND, # x options
				Gtk.AttachOptions.SHRINK, # y options
				5,5) # horizontal and vertical paddings
	...
	def on_initialize_fields(self):
		...
		flag = self._settings.get_update_with_synctex_flags()
		self._ui_update_synctex.set_active(flag)
	...
	def on_save_changes(self):
		...
		update_synctex = self._ui_update_synctex.get_active()
		self._settings.set_update_with_synctex_flags(update_synctex)
