...
my $EXTENDED_WARNING_CODE = <<'ENDOFTEX';
	...
	\global\DeclareRobustCommand{\GenericWarning}[2]{%
		...
		\fi%
		{\filename@parse \autolatex@@@currentfile}%
			...
			while (*LOGFILE && ($line = <LOGFILE>) && !$fatal_error) {
				...
				if ($is_empty_line) {
					...
					if ($current_log_block) {
						...
						if ($current_log_block =~ /^\!\s*\Q==>\E\s*fatal\s+error/si) {
							$fatal_error = "\Q!\E[^!]";
						}
						elsif ($current_log_block =~ /^(.+:[0-9]+:)\s*\Q==>\E\s*fatal\s+error/si) {
							$fatal_error = $1;
						}
						else {
					...
					# End of a sentence => add to current block
					$current_log_block .= $line;
				}
				else {
					# The carriage return character may be removed
					$line =~ s/[\n\r]+//s;
					$current_log_block .= $line;
				}
			}
					...
					if ($candidate && -e "$candidate") {
						...
						while (!$extracted_message && $i<@log_blocks) {
							...
							if ($block =~ /$candidate_pattern(.*)$/s) {
								my $message_text = $1 || '';
								$extracted_message = trim($candidate.$post.$message_text);
							}
							$i++;
						}
					}
