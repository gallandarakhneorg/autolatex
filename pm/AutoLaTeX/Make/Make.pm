...
my $EXTENDED_WARNING_CODE = <<'ENDOFTEX';
	...
	\global\DeclareRobustCommand{\GenericWarning}[2]{%
		...
		\ifx\autolatex@@@mainfile\autolatex@@@currentfile%
			...
			\advance\autolatex@@@lineno ::::AUTOLATEXHEADERSIZE::::\relax%
		\else%
					...
					if ($candidate && -f "$candidate") {
						...
						while (!$extracted_message && $i<@log_blocks) {
							...
							if ($block =~ /$candidate_pattern(.*)$/s) {
								my $message_text = $1 || '';
#								if ($self->{'is_extended_warning_enable'}) {
#									my $bn = basename($candidate);
#									if ($bn eq 'autolatex_autogenerated.tex') {
#										$candidate = $file;
#										$post =~ /^\:([0-9]+)\:$/;
#										my $oline_number = int($1);
#										my $hlines = countLinesIn($self->{'latex_warning_code'});
#										my $line_number = $oline_number - $hlines;
#										if ($line_number<1) {
#											$line_number = 1;
#										}
#										$post = ":$line_number:";
#										$message_text =~ s/^\Ql.$oline_number\E/l.$line_number/mg;
#										$message_text =~ s/line\s+$oline_number/line $line_number/sg;
#									}
#								}
								$extracted_message = $candidate.$post.$message_text;
							}
							$i++;
						}
					}
...
#							if ($self->{'is_extended_warning_enable'}) {
#								my $hlines = countLinesIn($self->{'latex_warning_code'});
#								my $olinenumber = $linenumber;
#								my $linenumber -= $hlines;
#								if ($linenumber<1) {
#									$linenumber = 1;
#								}
#								$message =~ s/^\Ql.$olinenumber\E/l.$linenumber/mg;
#								$message =~ s/line\s+$olinenumber/line $linenumber/sg;
#							}
