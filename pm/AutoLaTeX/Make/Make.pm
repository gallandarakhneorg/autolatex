...
my $EXTENDED_WARNING_CODE = <<'ENDOFTEX';
	...
	\global\DeclareRobustCommand{\GenericWarning}[2]{
		...
		\edef\autolatex@generic@warning@beginmessage{!!!![BeginWarning]\@filef@und:\ifx\filename@ext\relax .tex\fi:\the\inputlineno: }
		\edef\autolatex@generic@warning@endmessage{!!!![EndWarning]\@filef@und}
		\def\MessageBreak{^^J#1}
		\set@display@protect
		\immediate\write\@unused{^^J\autolatex@generic@warning@beginmessage#2\on@line.^^J\autolatex@generic@warning@endmessage^^J}
		\endgroup
	}
	\makeatother
ENDOFTEX
...
sub runLaTeX($;$) : method {
	...
	do {
		...
		if ($self->{'is_extended_warning_enable'}) {
			...
			print OUTFILE $self->{'latex_warning_code'}."\n";
