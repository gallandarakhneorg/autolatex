...
class TeXParser(Parser):
	...
	* - for reading the text until the end of the current LaTeX context.
		...
		while sep:
			...
			if sep == '{':
				...
				if self.math_mode is None:
					...
					logging.warning(
						_T("you try to close with a '\$' a mathematical mode opened with '\\[' (%s:%d)"),
				...
				if isText or isMath:
					if self.math_mode is not None:
						if not isMath:
							logging.warning(
								_T("you cannot use in text mode the active character '%s', which is defined in math mode (%s:%d)"),
								sep, self.filename, lineno)
							if (sep is not None):
						...
						logging.warning(
							_T("you cannot use in math mode the active character '%s', which is defined in text mode (%s:%d)"), sep,
							self.filename, lineno)
						if sep is not None:
							self.observer.text(self, sep)
					else:
	...
	def __searchCmdTrans(self, name : str, lineno : int, special : bool = False):
		...
		logging.debug(_T("Found parameter definition for '%s': math=%s; text=%s (%s:%d)"), name,
			(math or '<undef>'), (text or '<undef>'), self.filename, lineno)
			...
			if r: # Eates a mandatory parameter
				...
				if r: # Eates an optional parameter
					...
					if r:
						...
						msg = re.sub('[\n\r]', '\\n', msg, 0, re.DOTALL)
						msg = msg.replace("\t", "\\t")
						logging.warning(_T("expected a TeX macro for expanding the macro %s, here: '%s' (%s:%d)"),
							macro, msg, self.filename, lineno)
						params.append({
								'eval' : 1,
								'text' : '' })
						rawparams += "\\"
